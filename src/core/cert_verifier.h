#ifndef KATHTTP_CERT_VERIFIER_H
#define KATHTTP_CERT_VERIFIER_H
...
/* Outcome of a verification attempt. `code` carries the kathttp_error
 * to surface on failure so callers can distinguish cert failures from
 * generic TLS / transport errors. */
struct VerifyResult {
    bool ok = false;
    int code = 0;        /* KATHTTP_ERR_* to surface on failure */

...
#define KATHTTP_LOG_ERR(...)                                                    \
    do {                                                                        \
        if (KATHTTP_LOG_LEVEL >= 1) fprintf(stderr, "kathttp[E] " __VA_ARGS__); \
    } while (0)
#define KATHTTP_LOG_WARN(...)                                                   \
    do {                                                                        \
        if (KATHTTP_LOG_LEVEL >= 2) fprintf(stderr, "kathttp[W] " __VA_ARGS__); \
    } while (0)
#define KATHTTP_LOG_INFO(...)                                                   \
    do {                                                                        \
        if (KATHTTP_LOG_LEVEL >= 3) fprintf(stderr, "kathttp[I] " __VA_ARGS__); \
    } while (0)
#define KATHTTP_LOG_DEBUG(...)                                                  \
    do {                                                                        \
        if (KATHTTP_LOG_LEVEL >= 4) fprintf(stderr, "kathttp[D] " __VA_ARGS__); \
    } while (0)
#endif

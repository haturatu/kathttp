...
bool ResponseCache::cacheable(std::string_view method,
...
bool ResponseCache::get(std::string_view method, std::string_view url,
                        Response &out) {
  if (method != "GET") return false;
...
void ResponseCache::put(std::string_view method, std::string_view url,
                        const Response &resp) {
  ...
  if (pos != std::string::npos) {
    ...
    if (eq != std::string::npos) {
      ...
      std::string v = cc.substr(eq + 1, end == std::string::npos
                                              ? std::string::npos
                                              : end - (eq + 1));
  ...
  lru_.push_front(Entry{std::string(key), resp,
                        static_cast<uint64_t>(std::time(nullptr)), max_age});

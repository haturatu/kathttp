...
uint64_t now_ns() { return timestamp_now_ns(); }
...
int get_new_connection_id_cb(ngtcp2_conn *, ngtcp2_cid *cid,
...
int get_new_connection_id2_cb(ngtcp2_conn *, ngtcp2_cid *cid,
...
int recv_crypto_data_cb(ngtcp2_conn *conn, ngtcp2_encryption_level level,
...
int handshake_completed_cb(ngtcp2_conn *conn, void *user_data) {
  (void)conn;
  return static_cast<QuicClient *>(user_data)->on_handshake_completed() ? 0
                                                                       : -1;
}
...
int recv_rx_key_cb(ngtcp2_conn *conn, ngtcp2_encryption_level level,
                   void *user_data) {
...
int stream_open_cb(ngtcp2_conn *, int64_t, void *) { return 0; }

int recv_stream_data_cb(ngtcp2_conn *conn, uint32_t flags, int64_t stream_id,
                        uint64_t, const uint8_t *data, size_t datalen,
                        void *user_data, void *) {
  (void)conn;
  return static_cast<QuicClient *>(user_data)->on_recv_stream_data(
             flags, stream_id, data, datalen)
             ? 0
             : -1;
}

int acked_stream_data_offset_cb(ngtcp2_conn *conn, int64_t stream_id, uint64_t,
                                uint64_t datalen, void *user_data, void *) {
  (void)conn;
  return static_cast<QuicClient *>(user_data)->on_acked_stream_data(stream_id, datalen) ? 0 : NGTCP2_ERR_CALLBACK_FAILURE;
}

int stream_close_cb(ngtcp2_conn *conn, uint32_t, int64_t stream_id,
                    uint64_t app_error_code, void *user_data, void *) {
  (void)conn;
  return static_cast<QuicClient *>(user_data)->on_stream_close(
             stream_id, app_error_code)
             ? 0
             : -1;
}

int stream_reset_cb(ngtcp2_conn *conn, int64_t stream_id, uint64_t,
                    uint64_t app_error_code, void *user_data, void *) {
  (void)conn;
  return static_cast<QuicClient *>(user_data)->on_stream_reset(stream_id,
                                                                app_error_code)
             ? 0
             : -1;
}

int stream_stop_sending_cb(ngtcp2_conn *conn, int64_t stream_id,
                           uint64_t app_error_code, void *user_data, void *) {
  (void)conn;
  return static_cast<QuicClient *>(user_data)->on_stream_stop_sending(
             stream_id, app_error_code)
             ? 0
             : -1;
}
...
int extend_max_stream_data_cb(ngtcp2_conn *, int64_t stream_id, uint64_t,
                              void *user_data, void *) {
  return static_cast<QuicClient *>(user_data)->on_extend_max_stream_data(
             stream_id)
             ? 0
             : -1;
}

int update_key_cb(ngtcp2_conn *, uint8_t *, uint8_t *,
                  ngtcp2_crypto_aead_ctx *, uint8_t *, ngtcp2_crypto_aead_ctx *,
                  uint8_t *, const uint8_t *, const uint8_t *, size_t,
                  void *user_data) {
  (void)user_data;
  return 0;
}

int path_validation_cb(ngtcp2_conn *, uint32_t, const ngtcp2_path *,
                       const ngtcp2_path *, ngtcp2_path_validation_result,
                       void *user_data) {
  (void)user_data;
  return 0;
}

int select_preferred_addr_cb(ngtcp2_conn *, ngtcp2_path *,
                             const ngtcp2_preferred_addr *, void *user_data) {
  (void)user_data;
  return 0;
}
...
int recv_stateless_reset_cb(ngtcp2_conn *,
                            const ngtcp2_pkt_stateless_reset *,
                            void *user_data) {
  (void)user_data;
  return 0;
}

int recv_stateless_reset2_cb(ngtcp2_conn *,
                             const ngtcp2_pkt_stateless_reset2 *,
                             void *user_data) {
  (void)user_data;
  return 0;
}

int recv_datagram_cb(ngtcp2_conn *, uint32_t, const uint8_t *, size_t,
                      void *user_data) {
  (void)user_data;
  return 0;
}
...
int dcid_status_cb(ngtcp2_conn *, uint32_t, const ngtcp2_cid *,
                    ngtcp2_connection_id_status, void *user_data) {
  (void)user_data;
  return 0;
}

int dcid_status2_cb(ngtcp2_conn *, uint32_t, const ngtcp2_cid *,
                     ngtcp2_connection_id_status, void *user_data) {
  (void)user_data;
  return 0;
}

int begin_path_validation_cb(ngtcp2_conn *, uint32_t, const ngtcp2_path *,
                              const ngtcp2_path *, void *user_data) {
  (void)user_data;
  return 0;
}

int version_negotiation_cb(ngtcp2_conn *, const ngtcp2_pkt_hd *,
                           const uint32_t *, size_t, void *user_data) {
  (void)user_data;
  return 0;
}
...
constexpr ngtcp2_callbacks kCallbacks = {
    ...
    .remove_connection_id =
        [](ngtcp2_conn *, const ngtcp2_cid *, void *) -> int { return 0; },
...
void QuicClient::generate_reset_token(uint8_t *token, const ngtcp2_cid *cid) {
  ...
  ngtcp2_crypto_generate_stateless_reset_token(
      token, stateless_reset_secret_.data(), stateless_reset_secret_.size(),
      cid);
}
...
bool QuicClient::setup_connection() {
  ...
  int rv = ngtcp2_conn_client_new_versioned(
      &conn_, &dcid, &scid, &path,
      quic_version_ ? quic_version_ : NGTCP2_PROTO_VER_V1,
...
int QuicClient::event_loop() {
  ...
  while (!stop_) {
    ...
    if (pfds[0].revents & POLLIN) {
      for (int i = 0; i < 16; ++i) {
        ssize_t n =
            sock_.recv(pkt, sizeof(pkt), from, fromlen, ecn);
        if (n <= 0) break;
        ngtcp2_pkt_info pi{};
        pi.ecn = ecn;
        int rv = ngtcp2_conn_read_pkt_versioned(
            conn_, const_cast<const ngtcp2_path *>(&path_),
...
void QuicClient::write_pending() {
  ...
  for (;;) {
    ngtcp2_pkt_info pi{};
    ngtcp2_ssize n = ngtcp2_conn_write_pkt_versioned(
...
bool QuicClient::on_recv_stream_data(uint32_t flags, int64_t stream_id,
...
bool QuicClient::on_stream_stop_sending(int64_t stream_id,
...
void QuicClient::notify_job_headers(Job *job, int status,
...
namespace kathttp {
void QuicClient::set_wakeup_fd(int fd) { wakeup_fd_ = fd; }
}  // namespace kathttp

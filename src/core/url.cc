...
bool parse_url(std::string_view raw, Url &out) {
  ...
  if (auth.size() >= 2 && auth.front() == '[') {
    ...
    if (p < auth.size() && auth[p] == ':') {
      unsigned value = 0; auto ps = auth.substr(p + 1);
      auto rc = std::from_chars(ps.data(), ps.data() + ps.size(), value);
      if (ps.empty() || rc.ec != std::errc{} || rc.ptr != ps.data()+ps.size() || value == 0 || value > 65535) return false;
    ...
    if (pcolon != std::string_view::npos) {
      ...
      unsigned value = 0; auto ps = auth.substr(pcolon + 1);
      auto rc = std::from_chars(ps.data(), ps.data() + ps.size(), value);
      if (ps.empty() || rc.ec != std::errc{} || rc.ptr != ps.data()+ps.size() || value == 0 || value > 65535) return false;
      out.port = static_cast<uint16_t>(value);
    } else {
  ...
  std::string_view rest =
      auth_end == std::string_view::npos ? "" : raw.substr(auth_end);

...
QuicClient *Engine::get_or_create_client(const Url &origin) {
  std::string key = policy_tag_ + "|" + origin.scheme + "://" +
                          origin.host + ":" + std::to_string(origin.port);
...
void Engine::execute(kathttp_request *req, int64_t request_id,
...
void kathttp_client_destroy(kathttp_client *client) {
  if (!client) return;
  try { auto *e = reinterpret_cast<kathttp::Engine *>(client); e->destroy(); delete e; }
  catch (...) {}
}

void kathttp_client_close(kathttp_client *client) {
  if (!client) return;
  try { reinterpret_cast<kathttp::Engine *>(client)->destroy(); } catch (...) {}
}

void kathttp_client_set_origin_policy(kathttp_client *client,
                                     const char *policy_tag) {
  if (!client) return;
  try { reinterpret_cast<kathttp::Engine *>(client)->set_origin_policy(policy_tag ? policy_tag : ""); }
  catch (...) {}
}

void kathttp_client_execute(kathttp_client *client, kathttp_request *request,

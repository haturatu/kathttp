        ...
        job = viewModelScope.launch {
            ...
            try {
                ...
                val response = client.execute(
                    KatHttpRequest(snapshot.method, snapshot.url.trim(), headers, body),
                )
...
internal fun parseHeaders(source: String): List<KatHttpHeader> =
    source.lineSequence().mapIndexedNotNull { index, line ->
        ...
        KatHttpHeader(trimmed.substring(0, separator).trim(), trimmed.substring(separator + 1).trim())
    }.toList()

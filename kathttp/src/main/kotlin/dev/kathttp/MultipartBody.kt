...
class MultipartBody private constructor(val bytes: ByteArray, val contentType: String) {
    ...
    class Builder {
        ...
        private val boundary = "kathttp-${UUID.randomUUID()}"
...
fun multipartRequest(
    ...
    return KatHttpRequest(method, url, listOf(KatHttpHeader("content-type", mp.contentType)), mp.bytes)
}
...
object FormBody {
    ...
    fun request(url: String, method: String = "POST", fields: Map<String, String>): KatHttpRequest =
        KatHttpRequest(method, url, listOf(KatHttpHeader("content-type", "application/x-www-form-urlencoded")), build(fields))
